...
def run_powershell(script_name, *args):
    ...
    try:
        ...
        if not output:
            return {'success': False, 'error': 'Ingen output från script'}
        
        try:
            return {'success': True, 'output': json.loads(output)}
        except json.JSONDecodeError:
            return {'success': False, 'error': f'Ogiltig JSON: {output[:200]}'}
    except subprocess.TimeoutExpired:
        return {'success': False, 'error': 'Timeout: Script took too long'}
...
def get_db_acknowledgements(rule_id=None, device_id=None):
    ...
    if rule_id and device_id:
        c.execute('''
            ...
            WHERE rule_id = ? AND device_id = ? AND (expires_at IS NULL OR expires_at > datetime('now'))
        ''', (rule_id, device_id))
    elif rule_id:
        c.execute('''
            SELECT * FROM acknowledgements 
            WHERE rule_id = ? AND (expires_at IS NULL OR expires_at > datetime('now'))
        ''', (rule_id,))
    else:
...
def cameras():
    ...
    if result['success']:
        cameras = result['output'] if isinstance(result['output'], list) else []
    else:
        cameras = []
...
def import_cameras():
    if request.method == 'POST':
        ...
        if file:
            ...
            if result['success']:
                flash(f'Importerade kameror från {filename}', 'success')
            else:
                flash(f'Fel vid import: {result.get("error", "Okänt fel")}', 'error')
...
def camera_report():
    ...
    if report_format == 'pdf':
        filepath = os.path.join(app.config['REPORTS_DIR'], f'camera_report_{timestamp}.pdf')
        generate_pdf_report(cameras, filepath, 'Kamera-rapport')
        return send_file(filepath, as_attachment=True)
    else:
        filepath = os.path.join(app.config['REPORTS_DIR'], f'camera_report_{timestamp}.html')
...
def audit():
    ...
    stats = {
        ...
        'critical': sum(1 for r in audit_results if r.get('severity') == 'Critical' and not r.get('acknowledged')),
        'warning': sum(1 for r in audit_results if r.get('severity') == 'Warning' and not r.get('acknowledged')),
        'info': sum(1 for r in audit_results if r.get('severity') == 'Info' and not r.get('acknowledged')),
        'acknowledged': sum(1 for r in audit_results if r.get('acknowledged'))
    }
...
def acknowledge_issue():
    ...
    c = conn.cursor()
    c.execute('''
        INSERT INTO acknowledgements (rule_id, device_id, comment, acknowledged_by)
        VALUES (?, ?, ?, ?)
    ''', (data['rule_id'], data['device_id'], data['comment'], data.get('acknowledged_by', 'Admin')))
    conn.commit()
    conn.close()
...
def audit_report():
    ...
    if report_format == 'pdf':
        filepath = os.path.join(app.config['REPORTS_DIR'], f'audit_report_{timestamp}.pdf')
        generate_pdf_report(audit_results, filepath, 'Best Practice Audit')
        return send_file(filepath, as_attachment=True)
    else:
        filepath = os.path.join(app.config['REPORTS_DIR'], f'audit_report_{timestamp}.html')
...
def users_report():
    ...
    if report_format == 'pdf':
        filepath = os.path.join(app.config['REPORTS_DIR'], f'users_report_{timestamp}.pdf')
        generate_pdf_report(users, filepath, 'Användare och Roller')
        return send_file(filepath, as_attachment=True)
    else:
        filepath = os.path.join(app.config['REPORTS_DIR'], f'users_report_{timestamp}.html')
        generate_html_report(users, filepath, 'Användare och Roller')
        return send_file(filepath, as_attachment=True)
...
def schedule_report():
    ...
    c = conn.cursor()
    c.execute('''
        INSERT INTO report_schedule (report_type, schedule, enabled)
        VALUES (?, ?, 1)
    ''', (data['report_type'], data['schedule']))
    conn.commit()
    conn.close()
...
def run_report():
    ...
    if report_type == 'cameras' or report_type == 'all':
        result = run_powershell('get_cameras.ps1')
        cameras = result['output'] if (result['success'] and isinstance(result['output'], list)) else []
        generate_html_report(cameras, os.path.join(app.config['REPORTS_DIR'], f'camera_report_{timestamp}.html'), 'Kamera-rapport')
    
    if report_type == 'audit' or report_type == 'all':
        result = run_powershell('audit.ps1')
        audit_results = result['output'] if (result['success'] and isinstance(result['output'], list)) else []
        generate_html_report(audit_results, os.path.join(app.config['REPORTS_DIR'], f'audit_report_{timestamp}.html'), 'Best Practice Audit')
    ...
    if report_type == 'users' or report_type == 'all':
        ...
        generate_html_report(users, os.path.join(app.config['REPORTS_DIR'], f'users_report_{timestamp}.html'), 'Användare och Roller')
